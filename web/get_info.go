...
func (s *User) FetchUserInfo() (*User, error) {
...
func (s *User) FetchUserInfoContext(ctx context.Context) (*User, error) {
	fmt.Printf("Retrieving user info: %s\n", speedTestConfigUrl)
	...
	s.IP = users.Users[0].IP
	s.Lat = users.Users[0].Lat
	s.Lon = users.Users[0].Lon
	s.Isp = users.Users[0].Isp
